...
if __name__ == "__main__":
    ...
    configuration["players_with_nfl_hydro_demand"] = ['u5'] #+ ['u8']
    configuration["players_with_nfl_heat_demand"] = [] #['u6']
    configuration["players_with_fl_elec_demand"] = ['u2']#['u2','u3']# + ['u7']
    configuration["players_with_fl_hydro_demand"] = []
    configuration["players_with_fl_heat_demand"] = []
    sensitivity_analysis_candidates = {
        ...
        'hp_cap': [0.8],
        'els_cap': [1],
        'num_households': [1000], #적정, oversupply, undersupply
        'nu_cop': [3.28],
        'c_su_G': [50],
        'c_su_H': [10],
    ...
    ip = True
    lp_relax, chp, brute_force= False, False, False
    for i, values in enumerate(itertools.product(*param_values)):
        ...
        if ip:
            ...
            results_u1245 = results_complete_ip
            import pickle
            with open(f'results_u1245_{sensitivity_analysis["day"]}.pkl', 'wb') as f:
                pickle.dump(results_u1245, f)
            print("=== {u2,u5} Coalition: u2 Operation ===")
            
            # import pickle
            # with open(f'results_ip_{sensitivity_analysis["day"]}.pkl', 'wb') as f:
            #     pickle.dump(results_complete_ip, f)
        # INSERT_YOUR_CODE
        # pickle 파일을 읽으려면 아래와 같이 하면 됩니다:
        # import pickle
        # with open('your_filename.pkl', 'rb') as f:
        #     data = pickle.load(f)
        if lp_relax:
