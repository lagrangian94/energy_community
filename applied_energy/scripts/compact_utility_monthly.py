...
class LocalEnergyMarket:
    ...
    def _store_model_data(self):
        ...
        self.model.data["vars"]["z_sd_G"] = self.z_sd_G
        ...
        for u in self.players_with_heatpumps:
            ...
            for t in self.time_periods:
                if t != 0:
                    ...
                    cons = self.model.addCons(
                        self.z_on_H[u,t] <= 0.0,
                        name=f"heatpump_initial_state_{u}_{t}"
                    )
                    self.heatpump_cons[f"heatpump_initial_state_{u}_{t}"] = cons
                    cons = self.model.addCons(
                        self.z_sd_H[u,t] >= 1.0,
                        name=f"heatpump_shut_down_{u}_{t}"
                    )
                    self.heatpump_cons[f"heatpump_shut_down_{u}_{t}"] = cons
