...
4. ω^LR을 믿을 만하게 내려면 EF gap은 1e-6으로 둬야 한다. CG는 기본값에서 ω를 2%까지 알면 멈추고(11절), pricing은 거기에 필요한 절대 gap으로 푼다. ω가 필요 없거나 더 정밀해야 하면 `--no-omega-tol`로 CG gap 1e-6만 쓴다.
...
긴 실행 몇 개(n=60의 dyn-SAR E, bundle simplex, dual 웜스타트 0.01)가 traceback 없이 도중에 끝났다(exit 127). 처음에는 같은 기계의 다른 세션이 프로세스를 멈춘 것으로 봤는데, 그 판단은 틀렸다. |Ω|=10 실행에서 bash가 **Segmentation fault**를 직접 보고했으므로 프로세스 내부 충돌로 봐야 한다. 병렬 pricing 스레드와 Gurobi의 조합이 의심되지만 원인은 모른다. `-X faulthandler`를 켠 뒤의 실행(|Ω|=10, 20, 여섯 번)에서는 재현되지 않았다. 해당 표의 "중단" 표시는 이것이다.

## 11. 시나리오 수를 늘리면 (n=60, |Ω| = 10, 20)

기본값(DOI, 부하 분산, EF·CG gap 1e-6, pricing 1e-4), 해시 시드 21. |Ω|마다 시나리오를 새로 뽑으므로 ω^LR은 서로 다른 인스턴스의 값이다.

| | \|Ω\|=5 | \|Ω\|=10 | \|Ω\|=20 |
|---|---|---|---|
| linking 행 | 720 | 1,440 | 2,880 |
| EF (gap 1e-6) | 약 10s | 492s | 864s |
| CG | 299s (485회) | 1,247s (587회) | 10,328s (1,136회) |
| master LP / pricing | 76 / 215s | 684 / 542s | 7,325 / 2,925s |
| LP 한 번 (후반) | 0.1~0.15s | 약 1s | 4~9s |
| ω^LR | 3.73 | 9.25 | 6.98 |

DOI가 degeneracy를 풀어도 LP 크기는 시나리오 수에 비례한다. |Ω|=20에서는 비영 원소 약 500만 개로 master LP가 다시 71%다. 시나리오를 2배로 늘리면 반복 수와 반복당 LP 비용이 함께 늘어 시간이 4~8배가 된다.

**pricing gap이 LB를 막는 문제.** |Ω|=10에서 처음에는 끝부분에서 멈췄다. RMP와 LB가 0.12 떨어진 채 수백 회 움직이지 않았고, 목표는 0.024였다. LB는 pricing MILP의 dual bound 합이다. 시나리오가 늘면 Gurobi가 상대 gap 1e-4에서 멈추는 일이 잦아져서, 60개의 여유 합(incumbent − bound)이 그 0.12를 거의 다 차지했다. 원래 규칙은 "컬럼이 하나도 안 나올 때"만 pricing gap을 좁혔다. 그런데 reduced cost −3.5e-5짜리 컬럼이 계속 나와서 한 번도 발동하지 않았다. 이제는 그 여유 합이 남은 거리(RMP − LB)의 절반을 넘으면, 컬럼이 나왔는지와 상관없이 해당 prosumer의 gap을 1/10로 좁힌다.

**ω 기준 종료** (`--omega-tol 0.02`, `--pricing-abs`, **기본값**; 끄려면 `--no-omega-tol`, `--no-pricing-abs`). 기존 CG는 목적값 기준 상대 gap(보통 0.1%)으로 멈춘다. 반면 우리가 보고하는 ω는 약 24,000짜리 값 두 개의 차이다. 그래서 정밀도를 ω 기준으로 정한다.

- `--omega-tol τ`: UB − LB ≤ τ·ω̂ 에서도 멈춘다. ω̂ = EF 값 − LB로, 참 ω 이상이다.
- `--pricing-abs`: pricing MILP를 절대 gap (CG 허용치)/(2n)으로 푼다. 60개 여유의 합이 허용치의 절반을 넘지 않으니 LB가 pricing 때문에 막히지 않는다.

| | \|Ω\|=10 CG | ω^LR | \|Ω\|=20 CG | ω^LR |
|---|---|---|---|---|
| 기본값 (1e-6) | 1,247s (587회) | 9.246 | 10,328s (1,136회) | 6.979 |
| τ = 2% | 1,013s (491회), −19% | 9.386 | — | — |
| τ = 2% + `--pricing-abs` | 951s (456회), −24% | 9.382 | 7,456s (815회), −28% | 7.082 |

n=6, |Ω|=3에서는 31초 → 24초였고, ω는 정확값 7.0056 대비 +1.8%다. ω 오차는 모든 경우 +1.5~1.9%로 2% 안이며, 항상 위쪽으로 치우친다(멈출 때 LB가 참값보다 낮다). 절감은 대부분 후반 라운드에서 나온다(|Ω|=20의 라운드 5~7: 3,330초 → 1,295초). 초반 라운드와 master LP 비용 자체는 그대로여서, |Ω|=20의 EF + CG는 여전히 약 2시간 20분이다.
