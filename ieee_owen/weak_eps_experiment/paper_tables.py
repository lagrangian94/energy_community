...
def col_results(n):
    ...
    # reason: a truncated search found a genuinely violating coalition but not
    # necessarily the worst one, so its excess is only a LOWER bound. Counting it as
    # "in the core" would be unfounded, and averaging it into eps(chi) would drag the
    # mean below the value it claims to report. The dropped count is printed above the
    # table so the omission is visible rather than silent.
    # A BLANK stab_certified is not a truncation: rows written before the column
    # existed carry no value, and the 6-prosumer sweep is 30 such rows whose
    # separations finished in under 8 s against a 3600 s budget. Only an explicit
    # False marks a day that hit its limit -- the same `.get('certified', True)`
    # default that stability_check.flatten_for_csv applies when writing them.
    cert = o.get('stab_certified')
    if cert is not None:
        trunc = cert.astype(str).str.lower().isin(['false', '0'])
        n_trunc = int((ex.notna() & trunc).sum())
        ex = ex.where(~trunc)
    else:
        n_trunc = 0
...
def main():
    ...
    print('% day counts per size: ' + ', '.join(
        f"n={n}: {R[n]['days']} days, {R[n].get('measured', 0)} with a certified separation"
        + (f" ({R[n]['truncated']} truncated, dropped)" if R[n].get('truncated') else '')
        for n in sizes))
