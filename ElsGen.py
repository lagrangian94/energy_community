...
def generate_electrolyzer(eff_type, els_cap, wind_el_ratio, c_min, c_sb, c_su_G, segments, N_u=1):
    # %% Define config
    config_dict = {
        'eff_type': eff_type, # Choose model for hydorgen production curve1 (1:HYP-MIL, 2: HYP-L, 3: HYP-SOC, 4: HYP_MISOC)
    ...
    El = {'El_cap_tot': El_cap_tot, # Total electrolyzer capacity [MW]
        ...
        'P_min':c_min, # % minimum load
        'P_sb': c_sb, # % Power consumption in stand-by state
        'C_cold': c_su_G # Cold startup cost  [EUR/MW]
        } 
    ...
    El.update({"p_els_cap": El["a"][-1]*els_cap + El["b"][-1]})
    return El